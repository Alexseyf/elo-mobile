import { Text, View, StyleSheet, StatusBar, TouchableOpacity, ScrollView, Platform, ActivityIndicator, Alert } from "react-native";
  ...
  const fetchProfessorId = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (userData.roles && Array.isArray(userData.roles) && !userData.roles.includes('PROFESSOR')) {
          Toast.show({
            type: 'error',
            text1: 'Acesso negado',
  ...
  const fetchTurmasProfessor = async (id?: number) => {
    try {
      ...
      const url = API_URL.endsWith('/')
        ? `${API_URL}professores/${idToUse}/turmas`
        : `${API_URL}/professores/${idToUse}/turmas`;
        ...
        <View style={styles.sectionContainer}>
          ...
          {turmas.length === 0 ? (
            <View style={styles.emptyContainer}>
              <Text style={styles.emptyText}>Você não tem turmas associadas</Text>
            </View>
          ) : (
            turmas.map((turma) => (
              <TouchableOpacity 
